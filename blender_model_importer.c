...
//
//   C algorithm to convert the Model header file into an OpenGL workable file. It was found
//   on the following site: <http://ksolek.fm.interiowo.pl/Blender/>.
...
void InitMesh(){
    ...
    for (i=0; i<OBJECTS_COUNT; i++) {
        ...
        glBufferData(GL_ARRAY_BUFFER, sizeof (struct vertex_struct) * vertex_count[i], &vertexs[vertex_offset_table[i]], GL_STATIC_DRAW);
...
void DrawMesh(unsigned int index) {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, (GLuint)vbo[index]);
